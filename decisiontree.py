...
class DecisionTreeClassifier(ClassifierMixin, BaseDecisionTree):
    ...
    def predict_proba(self, X, check_input=True):
        ...
        if self.n_outputs_ == 1:  # 葉ノード数が1個の場合
    ...
    # 推定値に信頼区間上限追加
    # 特に決定木における推定確率は学習元のデータに即しているため，信頼区間の決めつけがある．
    # ブートストラップの代わりに信頼区間を設けることで，パラメトリックな探索と活用のバランシングを考える．
    # ブートストラップとTSの近似がどのようになっているか改めて整理すべきかも
    # BTSのメリット：頑健性，スケーラビリティ
    def predict_proba_UCB(self, X, check_input=True):
        """Predict class probabilities of the input samples X.
        ...
        if self.n_outputs_ == 1:
            ...
            for k in range(self.n_outputs_):
                proba_k = proba[:, k, : self.n_classes_[k]]  # axis=2: 行動，　
                normalizer = proba_k.sum(axis=1)[:, np.newaxis]
                normalizer[normalizer == 0.0] = 1.0
                proba_k /= normalizer
                all_proba.append(proba_k)
